(() => {
  ...
  function markup(city, options = {}) {
    ...
    if(!data.length) return '<p class="hf-v2-muted">Keine Transporte oder geplanten Bestellungen.</p>';
    ...
    const first=Math.floor(Math.min(...data.map(row=>row.departureAbsMinute))/DAY), last=Math.floor((Math.max(...data.map(row=>row.arrivalAbsMinute))-1)/DAY), days=Array.from({length:last-first+1},(_,i)=>first+i);
