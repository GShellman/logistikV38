(() => {
  ...
  function markup(city, options = {}) {
    ...
    const events=placed.map(row=>{const width=100/row.laneCount,style=`top:${row.topPercent}%;height:max(${row.heightPercent}%,32px);left:calc(${row.lane*width}% + 2px);width:calc(${width}% - 4px)`; const route=`${cityName(row.fromCityId)} → ${cityName(row.toCityId)}`; const empty=row.kind==='return'?'Rückfahrt':row.kind==='reposition'?'Leerfahrt':(row.goodId||'Ware'); return `<details class="hf-v2-transport-calendar__event hf-v2-transport-calendar__event--${row.kind}" style="${style}"><summary><b>${esc(route)}</b><span>${esc(empty)}</span><small>${esc(vehicleName(row.vehicleType))} · ${time(row.departureAbsMinute)}–${time(row.arrivalAbsMinute)}</small></summary><dl><div><dt>Status</dt><dd>${esc(row.status)}</dd></div>${Number(row.amountKg)>0?`<div><dt>Ladung</dt><dd>${esc(`${Number(row.netKg??row.amountKg).toLocaleString('de-CH')} kg${Number(row.carrierCount)>0?` · ${Number(row.carrierCount).toLocaleString('de-CH')} Paletten`:''} · ${Number(row.grossKg??row.amountKg).toLocaleString('de-CH')} kg brutto`)}</dd></div>`:''}</dl></details>`}).join('');
